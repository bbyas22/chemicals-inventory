...
配置项：
  ...
  USER_PASSWORD            化学实验室普通用户首次登记密码
  ADMIN_PASSWORD           化学实验室管理员首次登记密码（大小写不敏感）
  PHYSICS_USER_PASSWORD    物理实验室普通用户首次登记密码
  PHYSICS_ADMIN_PASSWORD   物理实验室管理员首次登记密码
  输入哪个实验室的密码，就进入哪个实验室的独立界面与数据库
  （化学 chemicals.db / 物理 physics.db，数据完全隔离）。

两种填写方式（可同时使用，环境变量优先）：
- 直接改本文件复制出来的 config.py 中“在文件里填写”的值；
- 或通过环境变量注入，不修改文件（适合宝塔进程管理、Docker 等场景）：
    PORT / SILICONFLOW_API_KEY
    CHEM_USER_PASSWORD / CHEM_ADMIN_PASSWORD
...
# 化学实验室：普通用户 / 管理员首次登记密码（请务必改成你自己的密码，不要留空）
USER_PASSWORD_IN_FILE = ""
ADMIN_PASSWORD_IN_FILE = ""  # 大小写不敏感

# 物理实验室：普通用户 / 管理员首次登记密码（请务必改成你自己的密码，不要留空）
PHYSICS_USER_PASSWORD_IN_FILE = ""
PHYSICS_ADMIN_PASSWORD_IN_FILE = ""
...
_env_phys_user_pwd = (os.environ.get("PHYSICS_USER_PASSWORD") or "").strip()
_env_phys_admin_pwd = (os.environ.get("PHYSICS_ADMIN_PASSWORD") or "").strip()
...
PHYSICS_USER_PASSWORD = _env_phys_user_pwd or str(PHYSICS_USER_PASSWORD_IN_FILE).strip()
PHYSICS_ADMIN_PASSWORD = (_env_phys_admin_pwd or str(PHYSICS_ADMIN_PASSWORD_IN_FILE).strip()).upper()
