...
后端 Flask + SQLite（免安装数据库服务，数据就是程序目录下一个文件），前端原生 HTML / CSS / JavaScript（无构建步骤），手机浏览器也能用。
...
├── delete_db_linux.sh    # Linux 生产环境重置数据库脚本（删除并按当前格式重建空库，无需重启服务）
├── test_ai.py            # 硅基流动 API 连通性测试脚本（可选）
├── templates/            # 页面模板（登记 / 实时库存 / 登记记录，共用 base）
└── static/               # 前端 js / css
```

首次运行会自动在程序目录创建 `chemicals.db`。
...
启动横幅会打印端口、AI Key 是否已配置；密码为空或两个密码相同时会有警告。
...
| `USER_PASSWORD_IN_FILE` | 普通用户首次登记密码（**部署时务必修改**） | `CHEM_USER_PASSWORD` |
| `ADMIN_PASSWORD_IN_FILE` | 管理员首次登记密码，大小写不敏感（**部署时务必修改**） | `CHEM_ADMIN_PASSWORD` |
...
| POST | `/api/user/verify` | 首次使用登记校验（姓名 + 密码，返回普通用户 / 管理员角色） |
...
按提示输入 `y` 确认后，脚本会删除旧库并**按当前表结构立即重建空库**，**无需重启服务**。
